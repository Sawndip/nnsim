...
__global__ void integrate_synapses(float* x, float* u, float* exp_rec, float* exp_fac, float* U, 
	...
	if (c < Ncon){
		...
		if (syn_num_spk[c] < neur_num_spk[pre_syns[c]]){
			if (t >= delay[c] && spk_times[Nneur*syn_num_spk[c] + pre_syn[c]] == t - delay[c]){
...
__global__ void integrate_neurons(float* Vms, float* Ums,
		float* a, float* b, float* c, float* d, float* k,
	...
	if (n < Neur){
		...
		float Isyn_new = -(AMPA_Amuont[n] + y_psns[n])*(Vm[n] - Erev_exc[n]) - GABA_Amuont[n]*(Vm[n] - Erev_inh[n]);
		float Vm = Vms[n];
		float Um = Ums[n];
		v1 = (k[n]*(Vm - Vr[n])*(Vm - Vt[n]) - Um + Ie[n] + Isyn_new)*time_step/Cm[n];
		...
		Vms[n] = Vm + v3;
		Ums[n] = Um + u3;
		v4 = (k[n]*(Vms[n] - Vr[n])*(Vms[n] - Vt[n]) - Ums[n] + Ie[n] + Isyn[n])*time_step/Cm[n];

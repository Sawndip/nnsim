...
nnsim_pykernel.so: $(BUILD_DIR)py_neuronet.o $(BUILD_DIR)libkernel_api.so
	$(CC) -shared -Wall -pthread $(BUILD_DIR)py_neuronet.o -L. -L$(BUILD_DIR) -lkernel_api -o nnsim_pykernel.so -Wl,-rpath,.,-rpath,$(BUILD_DIR)

$(BUILD_DIR)py_neuronet.o:
	$(CC) -c -fPIC -Wall $(SRC_DIR)py_neuronet.cpp -Wwrite-strings -o $(BUILD_DIR)py_neuronet.o

$(BUILD_DIR)libkernel_api.so: $(BUILD_DIR)kernel_api.o
	$(CC) -shared -Wall $(BUILD_DIR)kernel_api.o -o $(BUILD_DIR)libkernel_api.so

$(BUILD_DIR)kernel_api.o:
	$(CC) -c -fPIC -Wall -O3 $(SRC_DIR)kernel_api.cpp -o $(BUILD_DIR)kernel_api.o
